...
- [基本数据类型](#基本数据类型)
  - [1 整数](#1-整数)
      ...
      - [（2）赋值运算符](#2赋值运算符)
      - [（3）位运算符](#3位运算符)
      - [（4）转进制 （int、bin、oct、hex）](#4转进制-intbinocthex)
      - [（5）判断奇偶（&1、%2）](#5判断奇偶12)
    - [1.2 浮点数](#12-浮点数)
      - [（1） 书写形式](#1-书写形式)
      - [（1） 最大值](#1-最大值)
    ...
    - [1.5 字符串](#15-字符串)
      ...
      - [（5）计数（.count）（子字符串）](#5计数count子字符串)
      - [（6）字符串格式化(format)](#6字符串格式化format)
      - [（7）比较](#7比较)
      - [（8）转义字符（\）](#8转义字符)
      - [（9）str  <==>  bytes](#9str----bytes)
      - [（10）str  <==>  list](#10str----list)
      - [（11）正则（re.search）](#11正则research)
      - [（12）判断字符串中是否有汉字](#12判断字符串中是否有汉字)
  - [2 类型判断](#2-类型判断)
      - [（1）类型（type、isinstance）](#1类型typeisinstance)
        - [1）type](#1type)
        - [2）isinstance](#2isinstance)
        - [type() 与 isinstance()区别：](#type-与-isinstance区别)
...
#### （4）代替（.replace）
```python
file1_new = file1.replace('.mp3', '.wav')
```
...
● 字符串、int、long、float
isinstance(data, (int, str, types.LongType, float))
● 时间类型
...
● mongo obJect
isinstance(data, bson.objectid.ObjectId)
